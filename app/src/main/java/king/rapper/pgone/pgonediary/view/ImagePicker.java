...
public class ImagePicker {
    ...
    protected CharSequence getTitle(Context context) {
        if (TextUtils.isEmpty(title)) {
            return context.getString(com.linchaolong.android.imagepicker.R.string.pick_image_intent_chooser_title);
        }
        return title;
    }
    ...
    private void handlePickImageInner(Activity activity, Fragment fragment, Uri imageUri) {
        ...
        String realPathFromUri = CaptureFileUtils.getRealPathFromUri(activity, imageUri);
        LogUtils.e("图片路径？？" + realPathFromUri + "");
        if (!TextUtils.isEmpty(realPathFromUri) && realPathFromUri.endsWith(".gif")) {
            callback.onCropImage(Uri.parse(realPathFromUri));
        } else {
            // 打开裁剪图片界面
