...
public class PickerView extends View {
    ...
    private void performSelect() {
        if (mSelectListener != null) {
            if (mCurrentSelected > -1 && mCurrentSelected < mDataList.size())
    ...
    private void init() {
        ...
        mPaint.setColor(getResources().getColor(R.color.AppPrimary));
        //第二个paint
    ...
    private void drawData(Canvas canvas) {
        ...
        if (mCurrentSelected < mDataList.size() && mCurrentSelected > -1)
    ...
    private void drawOtherText(Canvas canvas, int position, int type) {
        ...
        float baseline = (float) (y - (fmi.bottom / 2.0 + fmi.top / 2.0));
        int iPosition = mCurrentSelected + type * position;
        if (iPosition > -1 && iPosition < mDataList.size())
